...
def score(config, train_checkpoint_id, train_config):
    ...
    if config.test_dataset == 'Human_MADS':
        from datasets.Human_MADS_Loader import Human_MADS_Dataset

        mds_dataset = Human_MADS_Dataset(split='test', image_size=train_config.image_size, mask=config.mask, with_class_label= True)

        log.info('Dataset length', len(mds_dataset))
        loader = DataLoader(mds_dataset, batch_size=config.batch_size, num_workers=2, shuffle=False, drop_last=False)
        ...
        key_prefix = config['name'] if 'name' in config else 'Human_MADS'      
